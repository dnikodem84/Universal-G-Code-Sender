...
//        comobBoxProbeToolsetterPin.setRenderer(new PinSettingComboRenderer());
        JCheckBox checkboxCheckModeStart = new JCheckBox("Check Mode Start");
        JCheckBox checkboxHardStop = new JCheckBox("Hard Stop");
        JCheckBox checkboxProbeHardLimit = new JCheckBox("Probe Hard Limit");
        JPanel pnlMain = new JPanel(new GridLayout(10,1));
        
        pnlMain.add(makeProbePanel("Probe Pin", comboBoxProbePin));
        pnlMain.add(makeDescrLabel("This is the signal from the probe."));
        pnlMain.add(makeProbePanel("Toolsetter Pin", comobBoxProbeToolsetterPin));
        pnlMain.add(makeDescrLabel("This is an optional second probe."));
        pnlMain.add(checkboxCheckModeStart);        
        pnlMain.add(makeDescrLabel("This will force a probe check before a probe is started."));
        pnlMain.add(checkboxHardStop);
        pnlMain.add(makeDescrLabel("If true the axis will do a hard stop rather than decelerate. This can be used with fragile bits that might break with the overtravel needed for deceleration. It is likely to be less accurate at higher speeds where the motor might skip a few steps without deceleration."));        
        pnlMain.add(checkboxProbeHardLimit);
        pnlMain.add(makeDescrLabel("If true the probes will act like hard limits, trigger an alarm and immediately stop motion when triggered during non probing motion. This is to prevent accidental damage to probes."));
        return pnlMain;
